<template>
	<uni-popup ref="popup" type="bottom">
		<view class="select-box">
			<view class="hardline">
				<text class="picker-action" @click="onCancel" v-text="cancelText"></text>
				<!-- <text class="title" v-text="title"></text> -->
				<view class="picker-modal-footer">
					<view class="picker-modal-footer-info">
						<view class="picker-display">
							<!-- <text>当前选择</text> -->
							<text class="picker-display-text" @click="showDayPicker">{{BeginTitle}}</text>
							<view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100" hover-class="picker-display-link-active"
							 @click="onShowTimePicker('begin')">{{PickerTimeTitle}}</view>
							 <!-- @click="onShowTimePicker('begin')" -->
						</view>
					</view>
				</view>
				<text class="picker-action" @click="onConfirm" v-text="confirmText"></text>
			</view>
			<view class="picker">
				<!-- 日期选择器 -->
				...
				<!-- 时间选择器 -->
				<view v-show="showTimePicker" class="picker">
...
<script>
	...
	export default {
		...
		computed: {
			...
			PickerTimeTitle() {
				this.BeginTitle != '未选择' ? DateTools.formatTimeArray(this.beginTime, this.showSeconds) : '';
...
<style lang="scss" scoped>
	...
	$calendar-height: 460rpx;
	$calendar-item-width: 90rpx;
	$calendar-item-height: 60rpx;
	...
	.picker {
		...
		&-calendar {
			...
			&-view {
				...
				&-bgbegin {
					border-radius: $calendar-item-height 0 0 $calendar-item-height;
					top: 10%;
					left: 10%;
					width: 90%;
				}
				...
				&-bgend {
					border-radius: 0 $calendar-item-height $calendar-item-height 0;
					top: 10%;
					left: 0%;
					width: 90%;
				}
				...
				&-item {
					...
					width: 90%;
					height: 90%;
					border-radius: $calendar-item-height;
					display: flex;
					align-items: center;
	...
	.picker-display-text .picker-display-link {
		font-size:32rpx;
	}
	.picker-display-link {
		color: #333333;
